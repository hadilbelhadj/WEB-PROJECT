...
export default class EditorComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.route.snapshot.params["slug"]) {
      combineLatest([
        ...
        .subscribe(([article, currentUser]) => { // Modification ici
          if (currentUser.nom === article.author.username) { // Modification ici
  ...
  submitForm(): void {
    ...
    this.articleService
      ...
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (article) => this.router.navigate(["/api/articles/", article.slug]),
        error: (err) => {
          this.errors = err;
