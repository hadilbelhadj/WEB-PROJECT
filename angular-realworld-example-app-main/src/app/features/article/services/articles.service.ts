...
export class ArticlesService {
  ...
  get(slug: string): Observable<Article> {
    return this.http
      .get<{ article: Article }>(`/api/articles/${slug}`)
      .pipe(map((data) => data.article));
  }

  delete(slug: string): Observable<void> {
    return this.http.delete<void>(`/api/articles/${slug}`);
  }

  create(article: Partial<Article>): Observable<Article> {
    return this.http
      .post<{ article: Article }>("/api/articles/", { article: article })
      .pipe(map((data) => data.article));
  }

  update(article: Partial<Article>): Observable<Article> {
    return this.http
      .put<{ article: Article }>(`/api/articles/${article.slug}`, {
        article: article,
      })
      .pipe(map((data) => data.article));
  }
